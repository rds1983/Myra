...
namespace Myra.Utility
{
    static class ExpressionParser
    {
        ...
        public static void Parse(Widget widget,List<Widget> widgets, int level = 0)
        {
            ...
            widget.Left = GetValueByExpression(widget.Layout2d.PositionXExpression, widgets,widget.Parent ,level);
            if (widget.Layout2d.PositionYExpression != "NULL")
                widget.Top = GetValueByExpression(widget.Layout2d.PositionYExpression, widgets, widget.Parent, level);
            if (widget.Layout2d.SizeYExpression != "NULL")
                widget.Height = GetValueByExpression(widget.Layout2d.SizeYExpression, widgets, widget.Parent, level);
            if (widget.Layout2d.SizeXExpression != "NULL")
                widget.Width = GetValueByExpression(widget.Layout2d.SizeXExpression, widgets, widget.Parent, level);
        ...
        static private int GetValueByExpression(string Expression, List<Widget> widgets, Widget parent, int level) {
            ...
            List<string> operators = Expression.Split(' ').ToList();
                ...
                float val = ParseOperator(operators[0], widgets, parent, level);
        ...
        static private float ParseOperator(string Expression, List<Widget> widgets, Widget parent,int level) 
        {
            ...
            if (Expression.Contains("&."))
            {
               ...
               return (float)GetValue(Expression.Split('.')[1], parent);
            }
            ...
            if (Expression.Contains("["))
            {
                ...
                return  (float)GetValue(Expression.Split('.')[1], widget);
            }
            return 0;
        }
        ...
        /*.h*/
        static private int? GetValue(string WalueName, Widget widgets) 
        {
            switch (WalueName)
            {
                case "l":
