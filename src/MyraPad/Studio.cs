            ...
            while (true)
            {
                ...
                if (_loadQueue.TryDequeue(out data))
                {
                    try
                    {
                        ...
                        if (styleException != null)
                        {
                            newProject.Diagnostics.Add(new MMLDiagnostic(MMLDiagnosticSeverity.Error, "", "", string.Format(
                                "Failed to load Stylesheet: '{0}'", styleException.Message)));
                        }
